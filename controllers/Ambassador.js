...
exports.create = async (req, res) => {
	try {
		if (req.body.picture) {
			req.body.picture = await file.upload(req.body.picture, '', constant.FITSTAR_BUCKET.ambassador);
		}
...
exports.update = async (req, res) => {
	try {
		if (req.body.picture) {
			req.body.picture = await file.upload(req.body.picture, '', constant.FITSTAR_BUCKET.ambassador.ambassador);
		}
...
exports.get = async (req, res) => {
	try {
		...
		let ambassador = await Ambassador.find({ isDeleted: false })
			...
			.exec();
		return res.status(200).send({ status: true, message: constant.SUCCESS, totalRecord, ambassador });
	} catch (error) {
		console.log('ERROR:::', error);
		return res.status(500).json({ status: false, message: error.message });
	}
...
exports.delete = async (req, res) => {
	try {
		...
		return res.status(200).send({ status: true, message: constant.DELETE_AMBASSADOR });
	} catch (error) {
		console.log('ERROR:::', error);
		return res.status(500).json({ status: false, message: error.message });
	}
