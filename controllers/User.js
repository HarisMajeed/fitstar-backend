...
exports.profileBasic = async (req, res) => {
	try {
		...
		if (req.body.image) {
			req.body.image = await file.upload(req.body.image, '', constant.FITSTAR_BUCKET.user);
		}
		let beforePromises = [];
		let afterPromises = [];
		...
		if (req.body.portfolio) {
			beforePromises = req.body.portfolio.map(async (item) => {
				return file.upload(item.imageBefore ? item.imageBefore : '', '', constant.FITSTAR_BUCKET.user);
			});
			...
			afterPromises = req.body.portfolio.map(async (item) => {
				return file.upload(item.imageAfter ? item.imageAfter : '', '', constant.FITSTAR_BUCKET.user);
			});
...
exports.create = async (req, res) => {
	try {
		...
		if (getUser) {
			return res.status(400).send({ status: false, message: constant.ALREADY_REGISTERED });
		}
		...
		if (req.body.image) {
			req.body.image = await file.upload(req.body.image, '', constant.FITSTAR_BUCKET.sponsor);
		}
		...
		return res.status(201).json({ status: true, message: constant.CREATE_USER });
	} catch (error) {
...
exports.get = async (req, res) => {
	try {
		...
		let users = await User.find({ role: { $ne: 'admin' }, isDeleted: false })
			...
			.exec();
		return res.status(200).send({ status: true, message: constant.SUCCESS, totalRecord, users });
	} catch (error) {
		console.log('ERROR:::', error);
		return res.status(500).json({ status: false, message: error.message });
	}
...
exports.search = async (req, res) => {
	try {
		let users = await User.find({ fullName: req.params.search, isDeleted: false });
