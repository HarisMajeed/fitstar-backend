...
exports.create = async (req, res) => {
	try {
		if (req.body.featuredImageOne) {
			req.body.featuredImageOne = await file.upload(req.body.featuredImageOne, '', constant.FITSTAR_BUCKET.blog);
		}
		if (req.body.featuredImageTwo) {
			req.body.featuredImageTwo = await file.upload(req.body.featuredImageTwo, '', constant.FITSTAR_BUCKET.blog);
		}
		if (req.body.authorImage) {
			req.body.authorImage = await file.upload(req.body.authorImage, '', constant.FITSTAR_BUCKET.blog);
		}
...
exports.get = async (req, res) => {
	try {
		...
		let blogs = await Blogs.find({ isDeleted: false })
			...
			.exec();
		return res.status(200).send({ status: true, message: constant.SUCCESS, totalRecord, blogs });
	} catch (error) {
		console.log('ERROR:::', error);
		return res.status(500).json({ status: false, message: error.message });
	}
...
exports.getByCategory = async (req, res) => {
	...
	try {
		...
		if (req.params.category == 'All') {
			...
			return res.status(200).send({ status: true, message: constant.SUCCESS, blogs });
		}
...
exports.delete = async (req, res) => {
	try {
		...
		return res.status(200).send({ status: true, message: constant.DELETE_BLOG });
	} catch (error) {
		console.log('ERROR:::', error);
		return res.status(500).json({ status: false, message: error.message });
	}
...
exports.search = async (req, res) => {
	try {
        ...
        { authorName: { $regex: searchItem, $options: 'i' } }
      ]
		})
			.sort({ _id: -1 })
			.limit(parseInt(req.params.limit) || 10)
			.skip(parseInt(req.params.offset) - 1)
			.exec();
		return res.status(200).send({ status: true, message: constant.SUCCESS, totalRecords, blogs });
	} catch (error) {
		return res.status(500).json({ status: false, message: error.message });
	}
};
