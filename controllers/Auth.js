...
exports.signUpUser = async (req, res) => {
    try {
        ...
        return res.status(201).json({ status: true, message: constant.NEW_USER_CREATED + " " + req.body.email});
    }
    catch (error) {
...
exports.logout = async (req, res) => {
    try {
        ...
        if (!token || !token.startsWith('Bearer ')) {
            return res.status(401).json({ status: false, message: constant.TOKEN_ERR });
        }
        var accessToken = token.split(' ')[1];
        console.log("TOKEN:::::::::", accessToken);
        var decoded = jwt.verify(accessToken, process.env.JWT_SECRET);
        let updateUser = await User.updateOne({ _id: decoded.id }, { tokenStatus: false });
        // console.log("UPDATE RESPONSE::", updateUser);
        if (!updateUser) return res.status(500).json({ status: false, message: constant.SERVER_ERROR });
        return res.status(200).send({ status: true, message: constant.LOGOUT_SUCCESS })
    } catch (error) {
        console.log("ERROR:::", error);
        return res.status(500).json({ status: false, message: error.message });
    }
